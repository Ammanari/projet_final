...
if ($mysqli->connect_errno) {
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    } elseif (!filter_var($input_username, FILTER_VALIDATE_REGEXP, array("options" => array("regexp" => "/^[A-Z][a-zA-Z0-9]{8,}$/")))) {
    ...
    if (empty($fname_err) && empty($lname_err) && empty($username_err) && empty($password_err) && empty($cpassword_err)) {
      ...
      if ($stmt = mysqli_prepare($mysqli, $sql)) {
        mysqli_stmt_bind_param($stmt, "sss", $param_fname, $param_lname, $param_username);
        ...
        if (mysqli_stmt_execute($stmt)) {
          ...
          header("location: formulaireAccueil.php");
          exit();
        } else {
  ...
  <div class="form-container">
    <form id="registrationForm" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
      ...
      <div class="form-group">
        ...
        <input type="button" name="login" value="Log In" onclick="window.location.href='login.html'">
      </div>
    </form>
  </div>
